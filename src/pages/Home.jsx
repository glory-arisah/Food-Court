	...
	return (
		<div>
			...
			<section className="bg-white py-8 px-6">
				<div className="categories text-[#232323] relative flex gap-x-8 mb-8 whitespace-nowrap overflow-x-auto font-semibold text-sm border-b-[0.5px] pb-2 border-[#ABBBC2] scrollbar">
				...
				<div className="flex justify-between items-center text-[#232323] font-semibold mb-8">
					<h4>
						...
						<span className="text-[#4094F7]">
							{meals.length} {meals.length === 1 ? 'result' : 'results'}
						</span>
					</h4>
					...
					>
						...
						<div
							className={`absolute right-0 top-full translate-y-1 z-10 min-w-[160px] space-y-2 p-1.5 rounded-lg bg-white text-left shadow-md transition-opacity duration-200 ${
							...
							{sortOrders.map((order) => (
								<div
									...
									className={`flex items-center justify-between ${
										sortOrder === order.key ? 'bg-[#F5F8FE]' : ''
									}`}
								>
									<button
										type="button"
										onClick={() => setSortOrder(order.key)}
										className={`text-left w-full p-1 hover:bg-[#F5F8FE] cursor-pointer										
									`}
									>
										{order.label}
									</button>
				...
				<div
					className={`fixed inset-0 z-20 flex items-end transition-opacity duration-300 ${
						isModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
					}`}
				>
					<div
						className={`bg-white rounded-t-3xl shadow-lg w-full max-w-lg mx-auto transform transition-transform duration-300 ease-out p-6 space-y-4 ${
							isModalOpen ? 'translate-y-0' : 'translate-y-full'
						}`}
					>
						<div className="flex justify-between items-center pb-2 border-b border-gray-200">
							<h3 className="text-xl font-bold">Sort by Name</h3>
						...
						<div className="space-y-2 text-[#232323] text-sm">
							{sortOrders.map((order) => (
								<div
									key={order.key}
									className={`flex items-center justify-between ${
								...
								>
									<button
										...
										className={`text-left flex-1 p-1 hover:bg-[#F5F8FE] cursor-pointer										
									`}
									>
										{order.label}
									</button>
						...
						<div className="pt-4">
							<button
								...
								className="w-full px-4 py-2.5 text-white cursor-pointer font-bold rounded-xl bg-[#3069FE] hover:bg-[#3069FE]/80 transition-colors duration-200"
							>
								Apply
							</button>
						</div>
